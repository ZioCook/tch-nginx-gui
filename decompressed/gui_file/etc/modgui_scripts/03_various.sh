...
check_upgrade_shit() {
  logecho "Fixing sysupgrade if needed..."
	if [ -f /lib/upgrade/resetgui.sh ]; then
		rm -f /lib/upgrade/resetgui.sh
		rm -f /lib/upgrade/transfer_bank1.sh
		rm -f /lib/upgrade/upgradegui.sh
		rm -f /lib/upgrade/checkver.sh
		rm -f /lib/upgrade/hardreset.sh
	fi
}
...
restore_original_mapper() {
  logecho "Restoring and enabling few transformer mappers..."
	...
	if [ -f "$orig_dir/device2/Device.map" ] && [ -f "$target/device2/Device.map" ] && [ "$(md5sum "$orig_dir/device2/Device.map" 2>/dev/null | awk '{print $1}')" != "$(md5sum "$target/device2/Device.map" 2>/dev/null | awk '{print $1}')" ]; then
		mkdir -p /tmp/tmp_bff_file
		cp $target/bbf/VoiceService* /tmp/tmp_bff_file/ 2>/dev/null
		rm -rf $target/bbf/*
		rm -rf $target/device2/*
		rm -rf $target/clash/*
		rm -rf $target/igd/*
		cp $orig_dir/bbf/* $target/bbf 2>/dev/null
		cp $orig_dir/clash/* $target/clash 2>/dev/null
		cp $orig_dir/device2/* $target/device2 2>/dev/null
		cp $orig_dir/igd/* $target/igd 2>/dev/null
		cp /tmp/tmp_bff_file/* $target/bbf 2>/dev/null
		rm -rf /tmp/tmp_bff_file
		logecho "Restored device-specific transformer mapping files"
	fi
	if [ -f "$orig_dir/bbf/VoiceService.VoiceProfile.Line.map" ]; then
		if [ ! -f "$target/bbf/VoiceService.VoiceProfile.Line.map" ]; then
			cp "$orig_dir/bbf/VoiceService.VoiceProfile.Line.map" "$target/bbf/"
		elif [ -f "$target/bbf/VoiceService.VoiceProfile.Line.map" ] && [ "$(md5sum "$orig_dir/bbf/VoiceService.VoiceProfile.Line.map" 2>/dev/null | awk '{print $1}')" != "$(md5sum "$target/bbf/VoiceService.VoiceProfile.Line.map" 2>/dev/null | awk '{print $1}')" ]; then
...
transformer_lib_check() {
  logecho "Transformer lib check"
	...
	if [ -f "$orig_dir/lib/lua/transformer/commitapply.lua" ] && [ -f "$target/lib/lua/transformer/commitapply.lua" ] && [ "$(md5sum "$orig_dir/lib/lua/transformer/commitapply.lua" 2>/dev/null | awk '{print $1}')" != "$(md5sum "$target/lib/lua/transformer/commitapply.lua" 2>/dev/null | awk '{print $1}')" ]; then
		rm -f $target/share/transformer/mappings/rpc/*
		rm -f $target/share/transformer/mappings/uci/*
		rm -rf $target/lib/lua/tch/*
		rm -rf $target/lib/lua/transformer/*
...
check_wansensing() {
	...
	if [ -d /usr/lib/lua/wansensingfw ]; then
		logecho "Old build detected, moving wansensing file"
		[ -f /usr/lib/lua/wansensing/scripthelpers.lua ] && rm /usr/lib/lua/wansensing/scripthelpers.lua
		[ ! -d /usr/lib/lua/wansensing ] && mkdir -p /usr/lib/lua/wansensing
		[ -f /usr/lib/lua/wansensingfw/scripthelpers.lua ] && mv /usr/lib/lua/wansensingfw/scripthelpers.lua /usr/lib/lua/wansensing/scripthelpers.lua
  ...
  logecho "Creating custom GUI utils symlinks..."
	if [ ! -h /usr/sbin/upgradegui ]; then
		...
		if [ -f "$upgrade_utility" ]; then
			ln -sf "$upgrade_utility" /usr/sbin/upgradegui
			ln -sf "$check_ver" /usr/sbin/checkver
		fi
	fi
}
...
update_checkver_upgrade_script() {
	if [ -f /usr/share/transformer/scripts/upgradegui.sh ]; then
	  logecho "Removing old update script"
		rm -f /usr/sbin/upgradegui
		rm -f /usr/sbin/checkver
		create_simbolic_utility
		rm -f /usr/share/transformer/scripts/upgradegui.sh
		rm -f /usr/share/transformer/scripts/checkver.sh
	fi
}
